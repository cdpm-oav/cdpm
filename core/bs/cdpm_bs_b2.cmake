...
function(_cdpm_b2_link_mode ctx_json out_link)
    ...
    if(NOT err)
        _cdpm_json_get(link_val "${options}" PATH "BOOST_LINK" OUT_TYPE link_type)
...
function(_cdpm_b2_libraries ctx_json out_args)
    ...
    string(JSON options ERROR_VARIABLE err GET "${ctx_json}" "options")
    if(NOT err)
        _cdpm_json_get(libs "${options}" PATH "BOOST_BUILD_LIBRARIES" OUT_TYPE libs_type)
...
function(_cdpm_b2_build_args ctx_json toolset variant am link libs out_args)
    ...
    set(with_icu FALSE)
    if(NOT err)
        _cdpm_json_get(with_icu_val "${options}" PATH "BOOST_WITH_ICU" OUT_TYPE with_icu_type)
    ...
    if(NOT err)
        _cdpm_json_get(cxxflags "${options}" PATH "BOOST_CXXFLAGS" OUT_TYPE cxxflags_type)
        if(NOT cxxflags_type STREQUAL "" AND NOT cxxflags STREQUAL "")
            list(APPEND args "cxxflags=${cxxflags}")
        endif()

        _cdpm_json_get(linkflags "${options}" PATH "BOOST_LINKFLAGS" OUT_TYPE linkflags_type)
