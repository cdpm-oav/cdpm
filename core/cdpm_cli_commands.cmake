...
function(cdpm_cmd_clean pkg_name pkg_hash)
    ...
    # Remove the matching ExternalProject build cache so the next install is not skipped by stale stamps.
    _cdpm_resolve_cli_runtime_dir(__runtime_dir "${__store}")
    string(TOLOWER "${pkg_name}" __pkg_name_lower)
    if(NOT pkg_hash STREQUAL "")
        set(__build_cache "${__runtime_dir}/bs/${__pkg_name_lower}-${pkg_hash}")
        if(EXISTS "${__build_cache}")
            file(REMOVE_RECURSE "${__build_cache}")
            message(STATUS "[cdpm] Removed build cache ${__build_cache}")
        endif()
    else()
        file(GLOB __build_cache_dirs LIST_DIRECTORIES true "${__runtime_dir}/bs/${__pkg_name_lower}-*")
        foreach(__build_cache IN LISTS __build_cache_dirs)
            if(IS_DIRECTORY "${__build_cache}")
                file(REMOVE_RECURSE "${__build_cache}")
                message(STATUS "[cdpm] Removed build cache ${__build_cache}")
            endif()
        endforeach()
